...
const AddNewAddressView = ({
  ...
  onClick={() => {
    newAddress({
      ...newAddress,
      isAddingNewAddress: false,
      value: "", // Reset the new address value
    });
  }}
>
  Cancel
...
const Checkout = () => {
  ...
  //     `${config.endpoint}/users/addresses`,
  ...
  //     `${config.enpoint}/user/addresses/${addressId}`,
  //     {
  //       header:{
  //         Authorization:`Bearer ${token}`,
  //       }
...
const deleteAddress = async (token, addressId) => {
  try {
    const response = await axios.delete(
      `${config.endpoint}/user/addresses/${addressId}`,  // Corrected
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const validateRequest = (items, addresses) => {
    ...
    //   enqueueSnackbar(
    //     "you do not have enough balanec in your wallet for this purchase",
    //     {
    //       variant:"warning",
    //     }
    //   );
    ...
    if (balance < totalCartValue) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "error", // Change the variant to "error" for an error message
        }
      );
      return false;
    }
    ...
    if(!addresses.all.length){
      enqueueSnackbar("please add anew address before proceding.",{
        variant:"waring",
      });
      return false;
    }
  ...
  //  }else{
  //   enqueueSnackbar(
  //     "could not place order,check that the backend is running,rechable ",
  //     {
  //       variant:"error",
  //     }
  //   );
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      // Update the local balance after a successful checkout
      const newBalance = parseInt(localStorage.getItem("balance")) - getTotalCartValue(items);
      localStorage.setItem("balance", newBalance);
      ...
      if (e.response) {
        ...
        enqueueSnackbar("Could not place order. Check backend connectivity.", {
          variant: "error",
        });
      }
...
useEffect(()=>{
  if(token){
    ...
    enqueueSnackbar("you must be logged in to acess check out page",{
      variant:"info",
    });
    history.push("/");
  }
