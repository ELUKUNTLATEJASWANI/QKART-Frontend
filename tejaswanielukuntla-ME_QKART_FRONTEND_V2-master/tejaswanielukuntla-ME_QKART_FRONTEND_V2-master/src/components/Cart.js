...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  //   const productData =productsData.find((product)=>item.productId===product._id);
  //   return{
  //     ...item,
  //     ...productData
  //   };
  // })
  // return nextCart;
  if (!cartData || !productsData) return []; // Check if either cartData or productsData is undefined or null
...
export const getTotalCartValue = (items = []) => {
  ...
  const total=items.reduce((total,item)=>total+(item.qty*item.cost),0);
  return total;
};
